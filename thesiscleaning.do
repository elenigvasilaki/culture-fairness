...
** Creation of the "Immigrants" sample
...
**                    Creation of regression variables
**                   ----------------------------------
...
**    Preferences in country of birth
**    -------------------------------
**    1) Weighted Mean Preference for Redistribution of natives in one's country of birth
gen wgincd=gincdif*dweight if brncntr==1 //natives only//
...
**	  Calculate the number of obs in each immigrant-birth country group
...
**   Preferences in country of parental origin
**  -------------------------------------------

** 	Mean Preferences for Redistribution in country of parental origin 
...
**  Creation of a dummy for the case that a mother is coming from an ESS-sample country and is an immigrant. 
gen m_eur=0
foreach cc of global countries{
 replace m_eur=1 if mbrncntb=="`cc'" & mbrncntb~=cntry
}

**  Creation of a dummy for the case that a father is coming from an ESS-sample country and is an immigrant. 
gen f_eur=0
foreach cc of global countries{
 replace f_eur=1 if fbrncntb=="`cc'" & fbrncntb~=cntry
}

**  Creation of a dummy for the case that both parents are coming from the same country
gen sameimm= (mbrncntb==fbrncntb) 

**  Creation of parsamp to sort R into three categories based on parental origin
...
**  which means that now it is the mean parental preference including native parents
replace ms_ginc=. if m_eur~=1
replace fs_ginc=. if f_eur~=1

**  Creation of variables to test whether the effect is stronger if both parents are ESS immigrants
...
**   GDP of parental source country
**   -------------------------------
...
drop temp1
**  The average of parents' home gdp for those parents who are immigrants
...
**  Inequality of parental source country
** ---------------------------------------
gen s_gini=.
...
**  Father's employment status when R was 14
recode emprf14 7/9=.
...
**  What type of organisation works/worked for
...
**  Alternative educational measures (Years of full-time education completed)
** ---------------------------------------------------------------------------
recode eduyrs 77/99=.
...
**  Household income (subjective feeling)
recode hincfel 7/9=. 
...
**   Show all languages by country if that languange is spoken by at least 1% of natives
capture drop temp
...
**  Immigrant density  
**  ------------------
**  the size of the birth country population by residence country
capture drop temp
capture drop temp2
...
**  Show the largest immigrant groups
list cntry cntbrth immdens if cntry~=cntbrth & _n<=100
drop minus_immdens

**  Again for 2nd generation immigrants
capture drop temp
capture drop temp2
egen temp=sum(dweight), by(cntry)
...
**   Creation of similar variable for 2nd generation individuals
...
**  Creation of dummy variables for being above/below the median
...
**  Creation of dummy variable for natives fraction greater than median 
...
** tagcc = use one observation for each Country of origin - Host country combination
